...
float TriggerSpeed[5] 	= {2.2f,			0.05f,			0.5f,		9000,			500};  //拨弹轮速度环
float TriggerAngle[5] 	= {0.2f,			0.0000f,			0.00f,		2000,			0};  //拨弹轮角度环
...
//			rc_shoot.trigger.target_angle-=angle;1
		}
	}
...
void shoot_task(void)
{
	...
	fric_pid();
	trigger_pid();
	if(trigger_back_flag>0 )
	{
		rc_shoot.trigger.target_angle-=((1*36*36)/360*8191)*0.25;
		trigger_back_flag=0;
		trigger_back_flag_cnt++;
	}
...
static void shoot_angle_clc(void)
{
	...
	if(heat>(heat_limit-20))//防止超热量
	{
		trigger_round=(((rc_shoot.trigger.total_angle-24114)*360)/(36*36*8191));
		rc_shoot.trigger.target_angle=((trigger_round*1*36*36)/360*8191)+24114.6;
	}
...
static void trigger_angle_set(void)
{
	...
	if(One_Shoot_flag==1)
	{
		rc_shoot.trigger.target_angle+=((SHOOT_NUM*1*36*36)/360*8191)+4914.6;
	}
	else 
	{
